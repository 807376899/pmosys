...
export default function ProjectFundingPanel({ project, onChanged }: { project: Project; onChanged: () => Promise<void> }) {
  ...
  async function save() {
    ...
    try { await apiPost(`/projects/${project.id}/funding-allocations`, { funding_source_id: Number(sourceId), allocated_amount: Number(amount), operator: "PMO办公室" }); setSourceId(""); setAmount(""); setError(""); await onChanged(); }
  ...
  return <section className="detail-section project-funding-panel">
    ...
    <div className="funding-project-add"><select className="select" value={sourceId} onChange={(event) => setSourceId(event.target.value)}><option value="">选择资金号</option>{sources.map((source) => <option value={source.id} key={source.id}>{source.fund_code} · {source.reference_amount == null ? "金额未记录" : `${formatCurrency(source.reference_amount)} 万`}</option>)}</select><input className="input" type="number" min="0" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="分配金额（万元）" /><button className="mini-button active" onClick={() => void save()}>保存分配</button></div>
